...
from .SimpleAgents.SleepAgent import SleepAgent
import numpy as np
import os
...
class MainAgent(PPOAgent):
    ...
    def load_bline(self):
        ckpt = os.path.join(os.getcwd(),"CybORG/Models","bline_train","100000.pth")
        return PPOAgent(52, self.action_space, restore=True, ckpt=ckpt,
                       deterministic=True, training=False)


    def load_meander(self):
        ckpt = os.path.join(os.getcwd(),"CybORG/Models","meander_train","100000.pth")
        return PPOAgent(52, self.action_space, restore=True, ckpt=ckpt,
                       deterministic=True, training=False)
