    ...
    def get_action(self, observation, action_space):
        ...
        while True:
            ...
            # Discover Remote Systems
                ...
                # pprint(observation)
                self.initial_ip = self.init_obs['User0']['Interface'][0]['IP Address']
                self.last_subnet = self.init_obs['User0']['Interface'][0]['Subnet']
                action = DiscoverRemoteSystems(session=session, agent=self.name, subnet=self.last_subnet)
            # Discover Network Services- new IP address found
                ...
                action =DiscoverNetworkServices(session=session, agent=self.name, ip_address=self.last_ip_address)

            # Exploit User1
            elif self.action == 2:
                 action = ExploitRemoteService(session=session, agent=self.name, ip_address=self.last_ip_address)
            ...
            # Privilege escalation on User Host
                ...
                action = PrivilegeEscalate(agent=self.name, hostname=hostname, session=session)
            ...
            # Exploit- Enterprise Host
                ...
                action = ExploitRemoteService(session=session, agent=self.name, ip_address=self.target_ip_address)
            ...
            # Scanning the new subnet found.
                ...
                action = DiscoverRemoteSystems(subnet=self.last_subnet, agent=self.name, session=session)
            ...
            # Discover Network Services- Enterprise2
                ...
                action = DiscoverNetworkServices(session=session, agent=self.name, ip_address=self.target_ip_address)
            ...
            # Discover Network Services- Op_Server0
                ...
                action = DiscoverNetworkServices(session=session, agent=self.name, ip_address=observation['Op_Server0']['Interface'][0]['IP Address'])

            # Exploit- Op_Server0
            elif self.action == 12:
                info = [value for key, value in observation.items() if key != 'success']
                if len(info) > 0:
                    action = ExploitRemoteService(agent=self.name, session=session, ip_address=info[0]['Interface'][0]['IP Address'])
                else:
                    self.action = 0
                    continue
            # Privilege escalation on Op_Server0
            elif self.action == 13:
                action = PrivilegeEscalate(agent=self.name, hostname='Op_Server0', session=session)
            # Impact on Op_server0
            elif self.action == 14:
                action = Impact(agent=self.name, session=session, hostname='Op_Server0')
